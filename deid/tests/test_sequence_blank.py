...
class TestSequenceBlank(unittest.TestCase):
    def setUp(self):
    ...
    def test_blank_single_named_field(self):
        print("Test BLANK on a single-occurrence named field.")
        field = "PatientName"
    ...
    def test_blank_single_tag_field(self):
        print("Test BLANK on a single-occurrence private field.")
        field = "00110009"
        dicom_file = next(get_files(self.dataset, pattern="ctbrain2.dcm"))
        recipe = create_recipe([{"action": "BLANK", "field": field}])
    ...
    def test_blank_one_level_one_occurrence(self):
        print("Test BLANK one level one occurrence")
        field = "00150002"
        dicom_file = next(get_files(self.dataset, pattern="ctbrain2.dcm"))
        recipe = create_recipe([{"action": "BLANK", "field": field}])
        ...
        self.assertEqual("", outputsequence[field].value)

    def test_blank_one_level_multiple_occurrences(self):
        print("Test BLANK one level multiple occurrences")
        field = "00150003"
        dicom_file = next(get_files(self.dataset, pattern="ctbrain2.dcm"))
        recipe = create_recipe([{"action": "BLANK", "field": field}])
        ...
        for sequencevalue in outputparent:
            self.assertEqual("", sequencevalue[field].value)

    def test_blank_multiple_levels_multiple_occurrences(self):
        print("Test BLANK multiple levels multiple occurrences")
        field = "00150006"
        dicom_file = next(get_files(self.dataset, pattern="ctbrain2.dcm"))
        recipe = create_recipe([{"action": "BLANK", "field": field}])
        ...
        for sequenceoccurrence in outputparent:
            for sequence2value in sequenceoccurrence:
                ...
                for level2dataset in level2value:
                    self.assertEqual("", level2dataset[field].value)

    def test_blank_nested_named_field(self):
        print("Test BLANK on a nested named field.")
