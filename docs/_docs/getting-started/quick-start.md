...
If you don't have patience for carefully reading each of the steps in the [getting started]({{ site.baseurl }}/getting-started/)
pages, this quick start is for you! Deid does two things:
...
In the first step, we check each image against a set of filters, with decreasing 
levels of specificity. An image is attributed to the filter group with the highest 
level of specificity, which is most specific to it. For example, an image that would 
be flagged with a general Blacklist criteria that is first flagged with Greylist 
(meaning we know how to clean it) is belongs to Greylist. We check more specific 
